-- 0049 WEEKLY HOURS — per-weekday opening hours an owner can set, overriding the
-- single open_time/close_time default. Stored as jsonb keyed by day-of-week
-- (0=Sun .. 6=Sat): { "1": {"open":"08:00","close":"18:00"}, "0": {"closed":true}, ... }.
-- An absent day key falls back to the shop default. create_booking/update_booking
-- reject bookings on a weekly-closed day (client also hides those dates).

alter table public.shops add column if not exists weekly_hours jsonb;
grant insert (weekly_hours) on public.shops to authenticated;
grant update (weekly_hours) on public.shops to authenticated;

-- True when the shop is closed on p_date by its weekly schedule (jsonb day map).
...
as $$
  select case
    ...
    else coalesce(((p_weekly -> (extract(dow from p_date)::int::text)) ->> 'closed')::boolean, false)
  end;
$$;
...
-- update_booking: also reject moving a booking onto a weekly-closed day.
