-- 0054 FREE-WASH VOUCHER SCOPE — the loyalty free-wash voucher should cover only
-- ONE standard car wash (the 'exterior' / "Car Wash" service), not the whole
-- booking. Any extra services selected are still charged to the wallet. Rewrites
-- create_booking + update_booking (preserving the custom-bookable-services logic)
-- with that change only. create_pending_booking is untouched — the card path
-- never takes a voucher.
...
begin
  ...
  if p_use_voucher then
    ...
    if not ('exterior' = any (p_service_ids)) then
      raise exception 'voucher requires a standard car wash';
    end if;
    -- The free wash covers ONE standard car wash; any extra services stay charged.
    v_free := true;
    v_total := greatest(0, v_total - coalesce((select price from public.services where id = 'exterior'), 0));
  end if;

  -- Wallet must cover whatever remains (a voucher booking can still owe for add-ons).
  ...
  if v_free then
    -- Voucher consumed (loyalty card restarts); still charge the wallet for any
    -- extra services beyond the free standard wash.
    if v_total > 0 then
...
begin
  ...
  -- Free-wash booking: the standard car wash stays free on edit too (only when a
  -- standard wash is still in the selection); extra services are charged.
  if v_booking.free_wash and 'exterior' = any (p_service_ids) then
    v_total := greatest(0, v_total - coalesce((select price from public.services where id = 'exterior'), 0));
  end if;
